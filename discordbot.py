...
class AlkalineClient(discord.Client):
    ...
    async def on_reaction_add(self, reaction, user):
        ...
        if user.id == self.user.id or user.bot or not reaction.message.channel.type is discord.channel.ChannelType.text:
            return
    ...
    async def on_message(self, message):
        ...
        # ignore messages from myself and from other bots and from private messages
        if message.author.id == self.user.id or message.author.bot or not message.channel.type is discord.channel.ChannelType.text:
            return
        # triggers plugin on_message plugin functions
        for mod in self.plugins:
            for plugin in self.plugins[mod]:
        ...
        if message.content.startswith(COMMAND_PREFIX) and len(
                message.content) > 1 and message.author.id != self.user.id:
            command = message.content.split(' ')[0][1:]
            for mod in self.plugins:
                if command in mod.commands:

                    # execute command only if user has permission or if command requires no permissions
                    if not 'perms' in mod.commands[command] or self.get_member_has_any_permissions(
                            mod.commands[command]['perms'], message.author.id):
                        for plugin in self.plugins[mod]:
                            await plugin.on_command(message, command,
                                                    message.content[1 + len(self.COMMAND_PREFIX) + len(command):])
                    else:
        ...
        if message.content.startswith(self.COMMAND_PREFIX):
            ...
            if message.content == COMMAND_PREFIX + 'modules' and can_core_commands:
                await message.channel.send("```%s```" % '\n'.join(
                        [k.__name__ + '\n\t' + ' '.join([a.__class__.__name__ for a in self.plugins[k]]) for k in
                         self.plugins]))
    ...
    async def unload_plugin_module(self, message):
        ...
        if len(candidates) > 1:
            await message.channel.send(
                'Please be more specific. Found candidates: %s' % ' '.join([x.__name__ for x in candidates]))
            return
        ...
        success_message = await message.channel.send(
            'Unloading module %s (has %i plugins) ...' % (mod.__name__, len(self.plugins[mod])))

        try:
            plugin_tasks = [task for task in asyncio.Task.all_tasks() if
                            hasattr(task, 'alkaline_identifier') and task.alkaline_identifier in [plugin.name for plugin
                                                                                                  in self.plugins[mod]]]
            if len(plugin_tasks) > 0:
                ...
                await success_message.edit(
                    content=success_message.content + ' stopping %i tasks...' % len(plugin_tasks))
